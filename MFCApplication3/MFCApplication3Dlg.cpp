...
void CMFCApplication3Dlg::OnBnClickedButton1()
{
	...
	server_in.sin_addr.s_addr = inet_addr("192.168.1.102");
...
u_short  g_iLenPkgHeader = sizeof(COMM_PKG_HEADER);
...
void CMFCApplication3Dlg::OnBnClickedButton4()
{
	...
	if (SendData(sClient, p_sendbuf, g_iLenPkgHeader) == SOCKET_ERROR)
	{
		AfxMessageBox(_T("SendData()失败"));
		//测试程序也就不关闭socket，直接返回去算了
		delete[] p_sendbuf; //内存总还是要释放的
	...
	char result[10000] = { 0 };
	sprintf_s(result, sizeof(result), "非常好，成功收到了心跳包的回应，收到字节数是%d个", rec);
	AfxMessageBox(result);
